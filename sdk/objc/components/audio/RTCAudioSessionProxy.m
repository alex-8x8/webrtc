...
- (BOOL)setCategory:(AVAudioSessionCategory)category
    ...
    if(self.setCategoryModeBlock) {
        NSError* error = self.setCategoryModeBlock(category, mode, options);
        if (outError) {
            *outError = error;
        }
        return (error == NULL);
    }
    return [self.native setCategory:category mode:mode options:options error:outError];
}
